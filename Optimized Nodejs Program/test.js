...
const returnArray = (products, prices) => {
    const pricingMap = new Map(prices.map(item => [item.sku, item]));
    const newProductArray = products.map(product => ({
        ...product,
        price: pricingMap.get(product.sku).price
    }));

    return newProductArray;
}

const result = returnArray(productsArray, pricingArray);
console.log(result);